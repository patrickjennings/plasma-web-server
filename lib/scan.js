        ...
        var buffer = fs.readFile( file, ( err, data ) => {
            id3.parse( data ).then( tags => {
                if( Object.keys( tags ).length > 0 ) {
                    ...
                    db.save_song_data( file, data ).then( results => {
                        callback();
                    } ).catch( err => {
                        callback( err );
                    } );
...
function scan_directory( directory, callback ) {
    var options = {
        cwd: directory,
        nosort: true,
        strict: true,
    ...
    glob( '*.mp3', options, ( err, files ) => {
        async.eachLimit( files, config.file_limit, parse_mp3_file, callback );
    } );
}
...
function scan_audio_files( directory ) {
    try {
        ...
        console.error( 'Could not parse directory.', directory );
        return;
    }

    var parse_unix_time = Math.round( new Date().getTime() / 1000 );

    scan_directory( directory, err => {
        if( err )
            return console.error( 'Failed to scan directory.', directory, err );

        console.log( 'Successfully scanned %s.', directory );

        db.delete_songs_from_directory( directory, parse_unix_time ).catch( err => {
            if( err )
                console.error( 'Failed to remove old records.', err );
        } );
    } );
}
